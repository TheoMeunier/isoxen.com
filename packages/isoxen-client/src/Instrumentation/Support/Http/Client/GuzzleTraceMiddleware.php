...
namespace Isoxen\Client\Instrumentation\Support\Http\Client;

use Closure;
use GuzzleHttp\Promise\PromiseInterface;
use Isoxen\Client\Facades\Meter;
use Isoxen\Client\Facades\Tracer;
use Isoxen\Client\Instrumentation\HttpClientInstrumentation;
        ...
        return static function (callable $handler): callable {
            return static function (RequestInterface $request, array $options) use ($handler) {
                ...
                $span = Tracer::newSpan(trim(sprintf('%s %s', $request->getMethod(), $route ?? '')))
                    ...
                    ->setAttribute(SpanType::ATTRIBUTE, SpanType::OutgoingRequest->value)
                    ->start();
