...
class Logger
{
    ...
    {
        if (config('isoxen.logs.inject_trace_id')) {
            unset($context[config('isoxen.logs.trace_id_field')]);
        }
        ...
        if (config('isoxen.user_context') === true && Auth::user() instanceof Authenticatable) {
