...
function Pagination({ className, ...props }: React.ComponentProps<'nav'>) {
    return (<nav role="navigation" aria-label="pagination" data-slot="pagination" className={cn('flex w-full justify-center', className)} {...props}/>);
}
...
function PaginationLink({ className, isActive, size = 'icon', href, children, ...props }: PaginationLinkProps) {
    ...
    if (!href) {
        return (<span aria-disabled="true" data-slot="pagination-link" className={classes}>
                {children}
            </span>);
    }
    return (<Link href={href} preserveScroll aria-current={isActive ? 'page' : undefined} data-slot="pagination-link" data-active={isActive} className={classes} {...props}>
            {children}
        </Link>);
}
